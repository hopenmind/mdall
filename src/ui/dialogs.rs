...
impl MdApp {
    ...
    /// The "Clean / Sanitize document" dialog. Audits the current Markdown, lets
    /// the user pick a mode (audit / sanitize / decontaminate), previews the
    /// effect, and applies it to the document source (undoable). Math zones and
    /// code are preserved by the purify core.
    pub(crate) fn show_purify_dialog(&mut self, ctx: &egui::Context) {
        use crate::ui::state::PurifyUiMode;
        use mdall_core::purify::{self, DocFormat, PurifyMode, PurifyOptions};

        let ui_mode = self.purify_dialog.mode;
        let core_mode = match ui_mode {
            PurifyUiMode::Audit => PurifyMode::Audit,
            PurifyUiMode::Sanitize => PurifyMode::Sanitize,
            PurifyUiMode::Decontaminate => PurifyMode::Decontaminate,
        };
        let opts = PurifyOptions {
            mode: core_mode,
            format: Some(DocFormat::Markdown),
            apply_fr_typography: self.purify_dialog.apply_fr_typography,
            apply_tic_removal: self.purify_dialog.apply_tic_removal,
            preserve_safelist: Vec::new(),
        };
        // Cheap for typical documents; recomputed each frame the dialog is open.
        let outcome = purify::purify_str(&self.source, None, &opts);
        let report = &outcome.report;

        let mut apply = false;
        let mut cancel = false;
        let mut open = self.purify_dialog.visible;

        egui::Window::new("Clean / Sanitize document")
            .open(&mut open)
            .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
            .resizable(false)
            .default_width(440.0)
            .show(ctx, |ui| {
                ui.label(
                    egui::RichText::new(format!(
                        "{} artifact(s) detected  -  {} watermark(s)",
                        report.artifacts_found, report.watermarks
                    ))
                    .strong(),
                );
                if report.artifact_breakdown.is_empty() {
                    ui.label(
                        egui::RichText::new("No LLM watermarks or encoding artifacts found.")
                            .size(11.5)
                            .color(theme::TEXT_2),
                    );
                } else {
                ...
                ui.radio_value(
                    &mut self.purify_dialog.mode,
                    PurifyUiMode::Audit,
                    "Audit  -  report only, no change",
                );
                ui.radio_value(
                    &mut self.purify_dialog.mode,
                    PurifyUiMode::Sanitize,
                    "Sanitize  -  strip watermarks, normalize encoding",
                );
                ui.radio_value(
                    &mut self.purify_dialog.mode,
                    PurifyUiMode::Decontaminate,
                    "Decontaminate  -  also remove LLM tics + FR typography",
                );
                if self.purify_dialog.mode == PurifyUiMode::Decontaminate {
                    ui.indent("purify_decon_opts", |ui| {
                        ui.checkbox(
                            &mut self.purify_dialog.apply_tic_removal,
                            "Remove LLM tics (fillers, hedges, robotic self-reference)",
                        );
                        ui.checkbox(
                            &mut self.purify_dialog.apply_fr_typography,
                            "French typography (NBSP before ;!?, guillemet spacing)",
                        );
                    });
                }

                ui.add_space(8.0);
                if ui_mode == PurifyUiMode::Audit {
                    ui.label(
                        egui::RichText::new(
                            "Frozen zones (code, YAML, front matter) and math ($...$, $$...$$) are always preserved.",
                        )
                        .size(11.0)
                        .color(theme::TEXT_MUTED),
                    );
                } else {
                    let a = report.applied.unwrap_or(0);
                    let s = report.skipped.unwrap_or(0);
                    let cb = report.chars_before.unwrap_or(0);
                    let ca = report.chars_after.unwrap_or(0);
                    ui.label(
                        egui::RichText::new(format!(
                            "Will change {} location(s); {} preserved (code / math).  {} -> {} chars.",
                            a, s, cb, ca
                        ))
                        .size(11.5)
                        .color(theme::TEXT_2),
                ...
                ui.add_space(10.0);
                ui.horizontal(|ui| {
                    let primary = if ui_mode == PurifyUiMode::Audit { "Close" } else { "Apply" };
        ...
        if !open || cancel {
            self.purify_dialog.visible = false;
            return;
        }
        if apply {
            self.purify_dialog.visible = false;
            if ui_mode != PurifyUiMode::Audit && outcome.text != self.source {
                self.prev_source = self.source.clone();
                self.source = outcome.text;
                self.segments_dirty = true;
                self.modified = true;
            }
        }
