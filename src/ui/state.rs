...
/// Purification (Clean / Sanitize) dialog mode, mirroring `purify::PurifyMode`.
#[derive(PartialEq, Clone, Copy)]
pub enum PurifyUiMode {
    /// Report only, never changes the document.
    Audit,
    /// Strip watermarks + normalize encoding (safe default).
    Sanitize,
    /// Also remove LLM tics + French typography (prose zones only).
    Decontaminate,
}

/// The "Clean / Sanitize document" dialog: pick a mode, preview the effect, apply.
pub struct PurifyDialog {
    pub visible: bool,
    pub mode: PurifyUiMode,
    pub apply_fr_typography: bool,
    pub apply_tic_removal: bool,
}

impl Default for PurifyDialog {
    fn default() -> Self {
        Self {
            visible: false,
            mode: PurifyUiMode::Sanitize,
