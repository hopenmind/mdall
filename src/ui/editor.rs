...
impl MdApp {
    ...
    pub(crate) fn show_source_editor(&mut self, ui: &mut egui::Ui) {
        // ── A4 page-view editor ──────────────────────────────────────────────
        // Grey "desktop" background, centered white A4 page with drop shadow.
        // A4 at 96 DPI ≈ 794 × 1123 px; content inset by 1-inch margins (72 px).
        ...
        // Fill the entire panel with the grey desktop colour.
        ui.painter().rect_filled(ui.max_rect(), 0.0, desktop_color);

        let available_w = ui.available_width();
        // Scale page down if the window is narrower than the A4 + padding.
        let page_w = PAGE_W.min(available_w - 16.0);

        // Graduated ruler fixed above the scroll area (drag handles set margins).
            ...
            .show(ui, |ui| {
                ...
                ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                    ...
                    page_frame.show(ui, |ui| {
                        ...
                        // Proportional font - writer feel, not code editor.
                        ...
                        // Apply programmatic cursor move requested by toolbar actions.
                            ...
                            let inner = egui::Frame::default()
                                ...
                                .show(ui, |ui| {
                                    self.render_rich_text_region_nav(
                                        ...
                                        focusable_neighbor(block_idx, 1),
                                        false, &mut block_op, f32::INFINITY,
                                    )
                                });
                                    ...
                                    match seg {
                                        InlineSeg::Text(r) => {
                                            ...
                                            self.render_rich_text_region_nav(
                                                ...
                                                None, None, false, &mut noop, cw + 3.0,
                                            );
                                        }
                        ...
                        _ => {
                                ...
                                editor::BlockKind::Paragraph | editor::BlockKind::Heading(_))
                            {
                                ...
                                self.render_rich_text_region_nav(
                                    ...
                                    focusable_neighbor(block_idx, -1),
                                    focusable_neighbor(block_idx, 1),
                                    true, &mut block_op, f32::INFINITY,
                                )
                            } else {
                                        ...
                                        ui.menu_button("Font Size \u{25b6}", |ui| {
                                            for sz in [10u8, 11, 12, 14, 16, 18, 20, 24, 28, 36] {
                                                if ui.button(format!("{} pt", sz)).clicked() {
                                                    self.wrap_text(&format!("<span style=\"font-size:{}pt\">", sz), "</span>");
                                                    ui.close_menu();
                                                }
                                            }
                                        });
...
impl MdApp {
    ...
    fn render_rich_text_region(
        ...
        self.render_rich_text_region_nav(
            ...
            None, None, false, &mut noop, f32::INFINITY,
        )
    }

    /// Aligned variant for centred / right-aligned single-line content (table
    /// columns, `<div text-align>`). egui's `LayoutJob.halign` centres around the
    /// origin (not within the cell), so instead we shrink the TextEdit to the
    /// content width and offset it with leading space. Multi-line / left content
    /// falls back to the normal full-width left rendering.
    fn render_rich_text_region_aligned(
        ...
        // Measure the content at the available width.
        let (content_w, single_line) = {
            ...
            (g.rect.width(), g.rows.len() <= 1)
        };
        if align == egui::Align::Min || !single_line || content_w + 2.0 >= avail {
        ...
        ui.horizontal(|ui| {
            ...
            self.render_rich_text_region_nav(
                ui, id, block_src, src_range, size, blocks, pending_change,
                None, None, false, &mut noop, content_w + 3.0,
            )
        }).inner
    }
...
fn draw_wavy_underline(painter: &egui::Painter, x0: f32, x1: f32, y: f32, color: egui::Color32) {
    if x1 <= x0 + 1.0 {
        return;
    }
    let amp = 1.3;
