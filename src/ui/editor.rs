...
impl MdApp {
    ...
    fn show_editor_segmented(&mut self, ui: &mut egui::Ui) {
        ...
        // Plan B step 3: document-level select-all (Ctrl+A), painted across blocks.
        // A pointer press clears it; consuming Ctrl+A here stops the focused block's
        // TextEdit from also running its own (single-block) select-all.
        ...
        egui::ScrollArea::both()
            ...
            page_frame.show(ui, |ui| {
                ...
                // Plan B step 3: paint the document selection over the cached block
                // rects (Ctrl+A highlights the whole document). Display-only for now;
                // a translucent tint so the text still reads through.
                if let Some(sel) = self.doc_selection {
                    let col = egui::Color32::from_rgba_unmultiplied(90, 140, 235, 55);
                    for h in &self.block_hits {
                        if sel.touches_block(h.idx) {
                            ui.painter().rect_filled(h.rect, 2.0, col);
                        }
                    }
                }
