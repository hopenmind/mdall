...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            // Cross-platform base: embed Typst's New Computer Modern so the UI has
            // a real serif and math coverage on Linux, macOS, or Windows without
            // Cambria. System fonts (below) layer on top and take priority.
            ...
            if let Some(bytes) = mdall_core::fonts::embedded_ui_math() {
                ...
                // Math/symbol family fallback when no system math font is present.
            ...
            // System fonts (Windows): Cambria as the preferred proportional face.
            // Skipped when MD2ALL_NO_SYSTEM_FONTS is set, which reproduces the
            // Linux/macOS font path on any OS for verification.
            ...
            if let Ok(data) = std::fs::read("C:\\Windows\\Fonts\\cambria.ttc") {
                ...
                // Cambria Math = face index 1 of cambria.ttc. A recognized OpenType
                // MATH font with broad Greek / operator / symbol coverage. Added as a
                // math-aware fallback (so equation/symbol glyphs render instead of tofu
                // boxes) and exposed as a Name family so equation rendering can prefer
                // it explicitly. (.ttc has 2 faces, so index 1 is always valid here.)
            ...
            // Load Cambria Bold for WYSIWYG **bold** rendering
            ...
            // Load Cambria Italic for WYSIWYG *italic* rendering (optional, egui synthesises if absent)
            ...
            app.prefs_snapshot = (app.app_lang.clone(), app.dark_mode, app.pdf_native);
...
struct MdApp {
    ...
    pdf_native: bool,
    /// Last-persisted (app_lang, dark_mode, pdf_native) snapshot, to detect changes.
    prefs_snapshot: (String, bool, bool),
...
impl Default for MdApp {
    fn default() -> Self {
        Self {
            ...
            app_lang: "en".into(),
            pdf_native: false,
            prefs_snapshot: ("en".into(), false, false),
            blocks: Vec::new(),
            segments_dirty: true,
            font_size: 14.0,
            zoom_level: 1.0,
...
impl eframe::App for MdApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let cur_prefs = (self.app_lang.clone(), self.dark_mode, self.pdf_native);
        if cur_prefs != self.prefs_snapshot {
            mdall_core::export::set_native_pdf(cur_prefs.2);
            crate::settings::save(&crate::settings::Settings {
                app_lang: cur_prefs.0.clone(),
                dark_mode: cur_prefs.1,
                pdf_native: cur_prefs.2,
            });
        ...
        ctx.set_pixels_per_point(self.base_ppp * self.zoom_level);
        ...
        if let Some(path) = path {
            if let Ok(data) = std::fs::read(&path) {
                ...
                if let Ok(sym) = std::fs::read("C:\\Windows\\Fonts\\seguisym.ttf") {
                    fonts.font_data.insert("Symbols".to_string(), egui::FontData::from_owned(sym));
                    if let Some(family) = fonts.families.get_mut(&egui::FontFamily::Proportional) {
                        family.push("Symbols".to_string());
                    }
                }
