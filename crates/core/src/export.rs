...
/// is used and the bundled general engine (Chromium Tier 1) is skipped. It is
...
pub fn export_pdf(markdown: &str, output_path: &Path, metadata: &PdfMetadata, source_dir: Option<&Path>) -> Result<(), String> {
    ...
    // Tier 1: bundled general engine (Chromium via CDP, perfect KaTeX, zero
    // system deps). Skipped when the Native converter is selected, so PDF stays
    // pure-Rust.
    if !native_pdf() {
        match crate::export_chromium::export_pdf_chromium(markdown, output_path, metadata, source_dir) {
            Ok(()) => return Ok(()),
            Err(e) => tier_errors.push(format!("Chromium: {}", e)),
        }
    }
...
// superseded by the bundled-Chromium CDP tier but retained as a fallback option.
#[allow(dead_code)]
...
fn inject_pdf_metadata(pdf_path: &Path, metadata: &PdfMetadata) -> Result<(), String> {
    ...
    info.set("Producer", Object::String(b"MD -> ALL (Rust/KaTeX/WebView2)".to_vec(), StringFormat::Literal));
