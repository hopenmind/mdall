...
<#
.SYNOPSIS
    Downloads the latest ungoogled-chromium portable x64 build and installs it
    into the chromium/ directory of the MD -> ALL project.

.DESCRIPTION
    Queries the ungoogled-chromium-windows GitHub releases API, downloads the
...
.PARAMETER Force
    Re-download even if chromium/ already exists.
...
Write-Host ""
Write-Host "  MD -> ALL — Chromium Setup" -ForegroundColor Cyan
...
if ((Test-Path $ChromeExe) -and -not $Force) {
    ...
    Write-Host "  [OK] ungoogled-chromium already installed: $existing" -ForegroundColor Green
...
Write-Host "  [OK] Installed: ungoogled-chromium $Version" -ForegroundColor Green
...
Write-Host "  The bundled Chromium is ready at: $ChromiumDir"
